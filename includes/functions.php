 ...
 * @return      void
 */
function timeapp_cancel_play() {
    // Don't process if nonce can't be verified
    if( ! wp_verify_nonce( $_GET['cancel-play'], 'cancel-play' ) ) return;
    ...
    $play       = get_post( $_GET['post'] );
    $date       = date( 'm-d-Y' );
    $filename   = $play->post_name . '-contract-cancelled-' . $date . '.pdf';
    ...
    $file = new TimeApp_Generate_PDF( $cache_dir . $filename, $_GET['post'], true );
    $file->build();
    ...
    // Send the email!
    $to         = $email;
    $subject    = sprintf( __( 'Time Music Agency Contract Cancelled - %1$s %2$s', 'timeapp' ), $artist->post_title, $start_date );
    $message    = timeapp_get_email_content( $play->ID );
    $headers[]  = 'From: Time Music Agency, Inc <contracts@timemusicagency.com>';
    $headers[]  = 'Cc: Danielle Findling <danielle@timemusicagency.com>';
